...
Filter out unrelated job titles using NLP:
...
The `job_posting_with_skills_filtered` is untidy since the `job_skills` variable contains a list of all the skills for the job posting. This makes it difficult to update the job posting with a new skill. Instead, let's make each observation represent a single skill for a job posting:

```{r}
# Make the data frame tidy - break up the `job_skills` variable so each observation is a job title/single skill combo
...
Please see `DATA 607 Project - Data Science Skills` document for findings and conclusions.